...
function PerformanceGraphs() {
  ...
  return <div className="mt-5 grid gap-5 lg:grid-cols-2">
    <section className="oe-card bg-[#133458] p-5 text-[#FAF7BB]">
       ...
       <h2 className="mt-1 font-display text-2xl text-[#000000]">North Indian Ocean test results</h2>
    ...
    <section className="oe-card bg-[#133458] p-5 text-[#FAF7BB]">
       ...
       <h2 className="mt-1 font-display text-2xl text-[#000000]">SST → +SSHa → +SSHa+SSS</h2>
       <div className="mt-5 flex items-center gap-4 text-[10px] text-[#000000]/70"><span className="flex items-center gap-2"><i className="h-2 w-5 bg-[#F26A38]" /> SST only</span><span className="flex items-center gap-2"><i className="h-2 w-5 bg-[#F4B51D]" /> + SSha</span><span className="flex items-center gap-2"><i className="h-2 w-5 bg-[#19A83A]" /> + SSS</span></div>
...
function ScientificProfileChart() {
  ...
  return <section className="oe-card overflow-hidden" data-testid="reconstruction-profile-chart"><div className="flex flex-wrap items-start justify-between gap-4 border-b border-[#D8D0B3] bg-[#f1edc9]/60 px-5 py-4"><div><div className="oe-kicker mb-1">Scientific profile</div><h2 className="font-display text-2xl text-[#133458]">Predicted vs. Observed Temperature Profile</h2></div><div className="flex flex-wrap items-center gap-4 text-[10px] text-[#536675]"><span className="flex items-center gap-2"><i className="h-0.5 w-5 bg-[#133458]" /> Actual (Argo measured)</span><span className="flex items-center gap-2"><i className="h-0.5 w-5 border-t-2 border-dashed border-[#D99B21]" /> Predicted (OceanEmbed)</span></div></div><div className="p-4 sm:p-6"><svg viewBox="0 0 680 460" className="h-auto w-full" role="img" aria-label="Predicted versus observed temperature profile with temperature on the horizontal axis and depth increasing downward"><rect x="70" y="43" width="570" height="340" fill="#FAF7BB" /><g stroke="#D8D0B3" strokeWidth="1"><line x1="70" y1="43" x2="640" y2="43" /><line x1="70" y1="124" x2="640" y2="124" /><line x1="70" y1="205" x2="640" y2="205" /><line x1="70" y1="286" x2="640" y2="286" /><line x1="70" y1="383" x2="640" y2="383" />{xTicks.map((tick) => <line key={tick} x1={xPosition(tick)} y1="43" x2={xPosition(tick)} y2="383" />)}</g><g stroke="#133458" strokeWidth="1.5"><line x1="70" y1="43" x2="70" y2="383" /><line x1="70" y1="383" x2="640" y2="383" /></g><g fill="#133458" fontFamily="Space Mono" fontSize="11" textAnchor="end">{yTicks.map((tick) => <text key={tick} x="59" y={yPosition(tick) + 4}>{tick}</text>)}</g><g fill="#133458" fontFamily="Space Mono" fontSize="11" textAnchor="middle">{xTicks.map((tick) => <text key={tick} x={xPosition(tick)} y="401">{tick}</text>)}</g><path d={observedPath} fill="none" stroke="#133458" strokeWidth="3.5" strokeLinecap="round" /><path d={predictedPath} fill="none" stroke="#D99B21" strokeWidth="2.75" strokeDasharray="7 5" strokeLinecap="round" /><text x="355" y="431" textAnchor="middle" fill="#133458" fontFamily="Space Mono" fontSize="11">Temperature (°C)</text><text x="18" y="215" textAnchor="middle" fill="#133458" fontFamily="Space Mono" fontSize="11" transform="rotate(-90 18 215)">Depth (m)</text></svg></div></section>;
}

function ReconstructionsScientific() {
  const [active, setActive] = useState('Arabian Sea');
  const runs = ['Arabian Sea', 'Bay of Bengal'];
  return <AppShell><div className="oe-page"><SectionHeading eyebrow={`Inference studio · ${studyRegion}`} title="Reconstructions" description="Inspect predicted and observed temperature profiles from the North Indian Ocean study field." action={<button className="oe-control oe-primary flex items-center gap-2" data-testid="button-start-run"><Play size={14} /> Run reconstruction</button>} /><div className="grid gap-5 xl:grid-cols-[230px_1fr]"><aside className="oe-card h-fit p-4"><div className="mb-3 flex items-center justify-between"><span className="oe-label">Study subregions</span><button className="text-[#838921]" aria-label="Add reconstruction" data-testid="button-add-reconstruction"><Plus size={16} /></button></div><div className="space-y-1">{runs.map((run, i) => <button onClick={() => setActive(run)} key={run} className={`w-full border-l-2 px-3 py-3 text-left ${active === run ? 'border-[#D99B21] bg-[#f1edc9]' : 'border-transparent hover:bg-[#f5f1d6]'}`} data-testid={`button-run-${i}`}><div className="text-xs font-semibold text-[#133458]">{run}</div><div className="mt-1 font-data text-[9px] text-[#8a927f]">{studyRegion}</div><div className="mt-2 flex items-center gap-1 text-[9px] text-[#838921]"><Check size={10} /> Complete</div></button>)}</div><button className="mt-4 flex w-full items-center justify-center gap-2 border-t border-[#D8D0B3] pt-4 text-xs text-[#536675]" data-testid="button-load-more-runs">Load archive <ChevronDown size={13} /></button></aside><section><div className="mb-4 flex flex-wrap items-center justify-between gap-3"><div><div className="oe-kicker mb-1">Selected reconstruction</div><h2 className="font-display text-2xl text-[#133458]">{active}</h2></div><div className="flex items-center gap-2"><span className="flex items-center gap-1.5 bg-[#e5ebd3] px-3 py-2 text-[10px] text-[#536b35]"><span className="h-1.5 w-1.5 rounded-full bg-[#838921]" /> COMPLETE</span><button className="oe-control p-2" aria-label="More reconstruction actions" data-testid="button-reconstruction-more"><MoreHorizontal size={16} /></button></div></div><ScientificProfileChart /></section></div></div></AppShell>;
}
