...
const depthValues = [
  { depth: '0–100 m', temp: '26.4', rmse: '0.54', score: 96.8 },
  { depth: '100–300 m', temp: '19.8', rmse: '0.67', score: 95.1 },
  { depth: '300–700 m', temp: '11.2', rmse: '0.79', score: 93.6 },
  { depth: '700–1,000 m', temp: '7.1', rmse: '0.91', score: 91.9 },
  { depth: '1,000–2,000 m', temp: '4.3', rmse: '1.12', score: 88.7 },
];
  ...
  return <>
    ...
    <aside className={`fixed inset-y-0 left-0 z-40 flex w-[248px] flex-col bg-[#133458] px-5 py-6 text-[#FAF7BB] transition-transform duration-300 md:translate-x-0 ${open ? 'translate-x-0' : '-translate-x-full'}`}>
      ...
      <div className="mt-auto rounded-sm border border-[#FAF7BB]/15 bg-[#FAF7BB]/[.06] p-4">
        ...
        <div className="mt-2 font-data text-[9px] tracking-wide text-[#FAF7BB]/45">MODEL V2.4.1 · SYNCED 4M AGO</div>
      </div>
      <div className="mt-5 flex items-center gap-3 border-t border-[#FAF7BB]/15 pt-5">
        <div className="grid h-8 w-8 place-items-center rounded-full bg-[#D99B21] text-xs font-semibold text-[#133458]">AN</div>
        <div><div className="text-xs">Avery Nori</div><div className="font-data text-[9px] text-[#FAF7BB]/45">Research lead</div></div>
...
function OceanMap({ compact = false, selectedLayer = 'Sea surface temperature', onSelect }: { compact?: boolean; selectedLayer?: string; onSelect?: (name: string) => void }) {
  const spots = [{ x: 29, y: 39, n: 'Arabian Sea', t: '28.4°C' }, { x: 52, y: 26, n: 'North Atlantic', t: '16.1°C' }, { x: 63, y: 56, n: 'South Pacific', t: '21.8°C' }, { x: 78, y: 38, n: 'Kuroshio', t: '24.9°C' }];
  return <div className={`relative overflow-hidden border border-[#294966] bg-[#133458] ${compact ? 'h-[300px]' : 'h-[490px]'}`}>
    ...
    <div className="absolute left-4 top-4 flex items-center gap-2 border border-[#FAF7BB]/15 bg-[#133458]/75 px-3 py-2 text-[10px] text-[#FAF7BB]/75 backdrop-blur"><span className="h-1.5 w-1.5 rounded-full bg-[#D99B21]" /> LIVE OBSERVATION FIELD <span className="font-data text-[#FAF7BB]/45">· 03 JUN 2024</span></div>
    <div className="absolute bottom-4 left-4 right-4 flex items-end justify-between"><div className="text-[10px] text-[#FAF7BB]/55"><div className="mb-1 font-data text-[#FAF7BB]/80">20°N</div><div>10°N</div><div>EQ</div></div><div className="w-40"><div className="mb-1 flex justify-between font-data text-[9px] text-[#FAF7BB]/65"><span>-2°C</span><span>{selectedLayer}</span><span>32°C</span></div><div className="h-1.5 bg-gradient-to-r from-[#2c6478] via-[#9caf68] to-[#d99b21]" /></div></div>
  </div>;
}
...
function Dashboard() {
  ...
  return <AppShell><div className="oe-page oe-grid"><SectionHeading eyebrow="Mission control · 03 June 2024" title="Ocean intelligence" description="A field view of what the ocean is telling us. Satellite observations resolved into a living picture of subsurface temperature." action={<button className="oe-control oe-primary flex items-center gap-2" data-testid="button-new-reconstruction"><Plus size={15} /> New reconstruction</button>} />
    <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4"><StatCard label="Reconstructed profiles" value="24,892" note="12.8% from last month" icon={Layers3} accent /><StatCard label="Model accuracy" value="94.7%" note="1.6% from last month" icon={Sparkles} /><StatCard label="Global RMSE" value="0.82°C" note="0.06°C from last month" icon={Activity} /><StatCard label="Active regions" value="128" note="7 new regions this cycle" icon={Globe2} /></div>
    <div className="mt-5 grid gap-5 xl:grid-cols-[1.55fr_1fr]"><section className="oe-card p-4 sm:p-5"><div className="mb-4 flex items-center justify-between"><div><div className="oe-kicker mb-1">Surface field</div><h2 className="font-display text-2xl text-[#133458]">Where the model is looking</h2></div><Link href="/map" className="flex items-center gap-1 text-xs font-semibold text-[#838921] hover:text-[#133458]" data-testid="link-open-full-map">Open full map <ArrowRight size={14} /></Link></div><OceanMap compact onSelect={setSelected} /><div className="mt-3 flex items-center justify-between text-xs text-[#536675]"><span>Selected region: <strong className="text-[#133458]">{selected}</strong></span><span className="font-data text-[10px]">+12.4°N, 54.8°E</span></div></section><section className="oe-card p-5"><div className="oe-kicker mb-1">At a glance</div><h2 className="font-display text-2xl text-[#133458]">Field notes</h2><div className="mt-6 space-y-5">{[['Arabian Sea', 'Strong thermal gradient detected at 300 m', '2 hours ago'], ['Equatorial Pacific', 'Reconstruction confidence rising', '5 hours ago'], ['North Atlantic', 'New profile cluster available', 'Yesterday']].map(([name, copy, time], i) => <div className="flex gap-3" key={name}><div className={`mt-1 h-2 w-2 shrink-0 rounded-full ${i === 0 ? 'bg-[#D99B21]' : 'bg-[#838921]'}`} /><div className="min-w-0"><div className="flex items-baseline justify-between gap-2"><div className="text-[13px] font-semibold text-[#133458]">{name}</div><div className="font-data text-[9px] text-[#8a927f]">{time}</div></div><p className="mt-1 text-xs leading-5 text-[#536675]">{copy}</p></div></div>)}</div><Link href="/reconstructions" className="mt-7 flex w-full items-center justify-center gap-2 border-t border-[#D8D0B3] pt-5 text-xs font-semibold text-[#838921]" data-testid="link-view-reconstructions">View reconstruction queue <ArrowRight size={14} /></Link></section></div>
    <div className="mt-5 grid gap-5 lg:grid-cols-[1fr_1fr]"><section className="oe-card p-5"><div className="mb-5 flex items-start justify-between"><div><div className="oe-kicker mb-1">Temporal signal</div><h2 className="font-display text-2xl text-[#133458]">Ocean temperature anomaly</h2></div><div className="flex items-center gap-2 text-[10px] text-[#536675]"><span className="h-2 w-2 rounded-full bg-[#133458]" /> Observed <span className="ml-2 h-2 w-2 rounded-full bg-[#9caf68]" /> Reconstructed</div></div><div className="h-44"><Chart /></div><div className="mt-1 flex justify-between font-data text-[9px] text-[#8a927f]"><span>−1.4°C</span><span>BASELINE 1993–2020</span><span>+2.1°C</span></div></section><section className="oe-card p-5"><div className="mb-5 flex items-start justify-between"><div><div className="oe-kicker mb-1">Depth transect</div><h2 className="font-display text-2xl text-[#133458]">Profile confidence</h2></div><Link href="/performance" className="text-xs text-[#838921]" data-testid="link-view-performance">View detail</Link></div><div className="h-44"><Chart variant="depth" /></div><div className="mt-2 flex justify-between text-[10px] text-[#536675]"><span>Surface</span><span>500 m</span><span>1,000 m</span></div></section></div>
  </div></AppShell>;
}
...
function MapPage() {
  ...
  const [selected, setSelected] = useState('Arabian Sea');
  const layers = ['Sea surface temperature', 'Subsurface reconstruction', 'Model confidence', 'Chlorophyll-a'];
  return <AppShell><div className="oe-page"><SectionHeading eyebrow="Spatial explorer" title="Ocean map" description="Move through the global field. Select a region to inspect its observation history and reconstruction confidence." action={<div className="flex gap-2"><button className="oe-control flex items-center gap-2" data-testid="button-map-download"><Download size={15} /> Export view</button><button className="oe-control p-2" aria-label="Map settings" data-testid="button-map-settings"><SlidersHorizontal size={16} /></button></div>} /><div className="grid gap-5 xl:grid-cols-[1fr_292px]"><section><OceanMap selectedLayer={layer} onSelect={setSelected} /><div className="mt-3 flex flex-wrap items-center justify-between gap-3 text-xs text-[#536675]"><span className="font-data text-[10px]">Showing 128 active regions · Updated 03 JUN 2024 09:42 UTC</span><span className="flex items-center gap-2"><span className="h-2 w-2 rounded-full bg-[#D99B21]" /> click a region to inspect</span></div></section><aside className="oe-card p-5"><div className="oe-kicker mb-2">Map controls</div><h2 className="font-display text-2xl text-[#133458]">Field layers</h2><div className="mt-5 space-y-2">{layers.map((name) => <button key={name} onClick={() => setLayer(name)} className={`flex w-full items-center gap-3 px-3 py-3 text-left text-xs transition-colors ${layer === name ? 'bg-[#133458] text-[#FAF7BB]' : 'bg-[#f1edc9] text-[#536675] hover:bg-[#e8e2ba]'}`} data-testid={`button-layer-${name.toLowerCase().replaceAll(' ', '-')}`}><span className={`h-2.5 w-2.5 rounded-full border ${layer === name ? 'border-[#D99B21] bg-[#D99B21]' : 'border-[#838921]'}`} />{name}{layer === name && <Check size={14} className="ml-auto text-[#D99B21]" />}</button>)}</div><div className="mt-8 border-t border-[#D8D0B3] pt-6"><div className="flex justify-between"><span className="oe-label">Depth slice</span><span className="font-data text-xs text-[#133458]">{depth} m</span></div><input type="range" min="0" max="2000" step="50" value={depth} onChange={(e) => setDepth(Number(e.target.value))} className="mt-4 w-full accent-[#838921]" data-testid="slider-map-depth" /><div className="mt-1 flex justify-between font-data text-[9px] text-[#8a927f]"><span>0 m</span><span>2,000 m</span></div></div><div className="mt-7 bg-[#133458] p-4 text-[#FAF7BB]"><div className="flex items-center gap-2 text-[10px] uppercase tracking-widest text-[#FAF7BB]/55"><CircleHelp size={13} /> Selected region</div><div className="mt-3 font-display text-xl">{selected}</div><div className="mt-3 grid grid-cols-2 gap-y-3 font-data text-[10px]"><span className="text-[#FAF7BB]/45">SURFACE</span><span className="text-right">28.4°C</span><span className="text-[#FAF7BB]/45">AT DEPTH</span><span className="text-right">12.1°C</span><span className="text-[#FAF7BB]/45">CONFIDENCE</span><span className="text-right text-[#D99B21]">96.2%</span></div><Link href="/reconstructions" className="mt-4 flex items-center gap-1 text-xs text-[#D99B21]" data-testid="link-inspect-region">Inspect region <ArrowRight size={13} /></Link></div></aside></div></div></AppShell>;
}

function Reconstructions() {
  const [active, setActive] = useState('Arabian Sea / 12°N 55°E');
  const [depth, setDepth] = useState(300);
  const runs = ['Arabian Sea / 12°N 55°E', 'Equatorial Pacific / 0°N 160°W', 'North Atlantic / 42°N 35°W'];
  return <AppShell><div className="oe-page"><SectionHeading eyebrow="Inference studio" title="Reconstructions" description="Inspect model-generated profiles beside their satellite evidence. Every run stays traceable to its source observations." action={<button className="oe-control oe-primary flex items-center gap-2" data-testid="button-start-run"><Play size={14} /> Run reconstruction</button>} /><div className="grid gap-5 xl:grid-cols-[230px_1fr]"><aside className="oe-card h-fit p-4"><div className="mb-3 flex items-center justify-between"><span className="oe-label">Recent runs</span><button className="text-[#838921]" aria-label="Add reconstruction" data-testid="button-add-reconstruction"><Plus size={16} /></button></div><div className="space-y-1">{runs.map((run, i) => <button onClick={() => setActive(run)} key={run} className={`w-full border-l-2 px-3 py-3 text-left ${active === run ? 'border-[#D99B21] bg-[#f1edc9]' : 'border-transparent hover:bg-[#f5f1d6]'}`} data-testid={`button-run-${i}`}><div className="text-xs font-semibold text-[#133458]">{run.split(' / ')[0]}</div><div className="mt-1 font-data text-[9px] text-[#8a927f]">{run.split(' / ')[1]}</div><div className="mt-2 flex items-center gap-1 text-[9px] text-[#838921]"><Check size={10} /> Complete · {i === 0 ? '2m' : '1d'} ago</div></button>)}</div><button className="mt-4 flex w-full items-center justify-center gap-2 border-t border-[#D8D0B3] pt-4 text-xs text-[#536675]" data-testid="button-load-more-runs">Load archive <ChevronDown size={13} /></button></aside><section><div className="mb-4 flex flex-wrap items-center justify-between gap-3"><div><div className="oe-kicker mb-1">Selected reconstruction</div><h2 className="font-display text-2xl text-[#133458]">{active}</h2></div><div className="flex items-center gap-2"><span className="flex items-center gap-1.5 bg-[#e5ebd3] px-3 py-2 text-[10px] text-[#536b35]"><span className="h-1.5 w-1.5 rounded-full bg-[#838921]" /> COMPLETE</span><button className="oe-control p-2" aria-label="More reconstruction actions" data-testid="button-reconstruction-more"><MoreHorizontal size={16} /></button></div></div><div className="oe-card overflow-hidden"><div className="flex flex-wrap items-center justify-between gap-4 border-b border-[#D8D0B3] bg-[#f1edc9]/60 px-5 py-4"><div className="flex items-center gap-5 text-xs"><span className="font-semibold text-[#133458]">Profile view</span><span className="text-[#8a927f]">Source evidence</span><span className="text-[#8a927f]">Diagnostics</span></div><div className="flex items-center gap-2 text-[10px] text-[#536675]"><span className="h-2 w-2 rounded-full bg-[#133458]" /> reconstructed <span className="ml-2 h-2 w-2 rounded-full bg-[#D99B21]" /> observed</div></div><div className="grid gap-6 p-5 lg:grid-cols-[1fr_250px]"><div className="relative h-[390px] border-l border-b border-[#D8D0B3]"><div className="absolute left-[-2px] top-0 h-full border-l-2 border-[#838921]" /><div className="absolute inset-0 flex flex-col justify-between py-1 font-data text-[9px] text-[#8a927f]"><span>0 m</span><span>500 m</span><span>1,000 m</span><span>1,500 m</span><span>2,000 m</span></div><svg viewBox="0 0 450 400" className="absolute inset-y-0 left-10 h-full w-[calc(100%-40px)]" preserveAspectRatio="none" aria-label="Depth reconstruction chart"><path d="M0 22 C80 35 100 58 138 85 S190 143 230 175 S276 220 305 259 S340 320 382 373" fill="none" stroke="#133458" strokeWidth="4" /><path d="M0 27 C70 38 110 67 145 94 S187 139 225 183 S278 232 310 269 S350 328 389 379" fill="none" stroke="#D99B21" strokeWidth="2.5" strokeDasharray="6 5" /><path d="M0 48 C70 57 102 86 144 116 S193 156 230 204 S280 255 310 286 S347 347 395 394" fill="none" stroke="#838921" strokeWidth="1.5" opacity=".5" /></svg><div className="absolute bottom-[-25px] left-10 right-0 flex justify-between font-data text-[9px] text-[#8a927f]"><span>2°C</span><span>10°C</span><span>18°C</span><span>26°C</span><span>30°C</span></div></div><div><div className="oe-label">Depth focus</div><div className="mt-3 font-data text-2xl text-[#133458]">{depth} <span className="font-sans text-xs text-[#536675]">metres</span></div><input type="range" min="0" max="2000" step="50" value={depth} onChange={(e) => setDepth(Number(e.target.value))} className="mt-3 w-full accent-[#D99B21]" data-testid="slider-reconstruction-depth" /><div className="mt-1 flex justify-between font-data text-[9px] text-[#8a927f]"><span>Surface</span><span>2,000 m</span></div><div className="mt-8 space-y-4 border-t border-[#D8D0B3] pt-5"><div><div className="oe-label">Temperature at focus</div><div className="mt-1 font-data text-lg text-[#133458]">{depth < 200 ? '26.4' : depth < 600 ? '12.1' : '6.8'}°C</div></div><div><div className="oe-label">Confidence interval</div><div className="mt-1 font-data text-lg text-[#133458]">± 0.82°C</div></div><div><div className="oe-label">Source profiles</div><div className="mt-1 font-data text-lg text-[#133458]">1,842</div></div></div></div></div></div></section></div></div></AppShell>;
}

function Temperature() {
  const [metric, setMetric] = useState('Temperature');
  const [period, setPeriod] = useState('12 months');
  return <AppShell><div className="oe-page oe-grid"><SectionHeading eyebrow="Thermal analytics" title="Temperature" description="Compare observed satellite signals with the reconstructed subsurface field across time, depth, and basin." action={<button className="oe-control flex items-center gap-2" data-testid="button-export-temperature"><Download size={15} /> Export CSV</button>} /><div className="mb-5 flex flex-wrap gap-2">{['Temperature', 'Anomaly', 'RMSE'].map((m) => <button key={m} onClick={() => setMetric(m)} className={`oe-control ${metric === m ? 'oe-primary' : ''}`} data-testid={`button-temperature-metric-${m.toLowerCase()}`}>{m}</button>)}<div className="ml-auto flex gap-2"><button className="oe-control flex items-center gap-2" data-testid="button-period"><RotateCcw size={14} /> {period}</button><button onClick={() => setPeriod(period === '12 months' ? '24 months' : '12 months')} className="oe-control p-2" aria-label="Change period" data-testid="button-change-period"><ChevronDown size={15} /></button></div></div><section className="oe-card p-5"><div className="flex flex-wrap items-start justify-between gap-4"><div><div className="oe-kicker mb-1">Global mean · {metric}</div><h2 className="font-display text-2xl text-[#133458]">Observed vs reconstructed</h2></div><div className="flex gap-5 text-[10px] text-[#536675]"><span className="flex items-center gap-2"><i className="h-2 w-5 bg-[#133458]" /> Observed</span><span className="flex items-center gap-2"><i className="h-2 w-5 bg-[#D99B21]" /> Reconstructed</span></div></div><div className="mt-6 h-[300px]"><Chart variant={metric === 'RMSE' ? 'bars' : 'line'} /></div></section><div className="mt-5 grid gap-5 lg:grid-cols-[1fr_1fr]"><section className="oe-card p-5"><div className="oe-kicker mb-1">Depth profile</div><h2 className="font-display text-2xl text-[#133458]">Arabian Sea transect</h2><div className="mt-5 space-y-3">{depthValues.map((row) => <div key={row.depth} className="grid grid-cols-[105px_1fr_55px] items-center gap-3 text-xs"><span className="font-data text-[10px] text-[#536675]">{row.depth}</span><div className="h-2 bg-[#e8e2ba]"><div className="h-full bg-[#838921]" style={{ width: `${row.score}%` }} /></div><span className="font-data text-right text-[#133458]">{row.temp}°</span></div>)}</div></section><section className="oe-card p-5"><div className="oe-kicker mb-1">Signal summary</div><h2 className="font-display text-2xl text-[#133458]">What changed</h2><div className="mt-5 divide-y divide-[#D8D0B3]">{[['Surface field', '+0.38°C', 'Warming above seasonal baseline'], ['Subsurface core', '−0.12°C', 'Cooler at 700–1,000 m'], ['Agreement', '94.7%', 'Across validated profiles']].map(([a,b,c], i) => <div key={a} className="flex items-center justify-between py-3"><div><div className="text-xs font-semibold text-[#133458]">{a}</div><div className="mt-1 text-[11px] text-[#536675]">{c}</div></div><div className={`font-data text-sm ${i === 1 ? 'text-[#838921]' : 'text-[#D99B21]'}`}>{b}</div></div>)}</div></section></div></div></AppShell>;
}

function Performance() {
  const [tab, setTab] = useState('By depth');
  return <AppShell><div className="oe-page"><SectionHeading eyebrow="Validation lab" title="Performance" description="Measure how the embedding model holds up across depth, basin, and competing reconstruction approaches." action={<Link href="/performance/architecture" className="oe-control flex items-center gap-2" data-testid="link-model-architecture"><Network size={15} /> Model architecture</Link>} /><div className="grid gap-4 sm:grid-cols-3"><StatCard label="Accuracy" value="94.7%" note="Validated against Argo" icon={Check} accent /><StatCard label="Global RMSE" value="0.82°C" note="Across 24,892 profiles" icon={Activity} /><StatCard label="Calibration" value="0.91" note="Reliability score" icon={Zap} /></div><section className="oe-card mt-5 p-5"><div className="flex flex-wrap items-center justify-between gap-3"><div><div className="oe-kicker mb-1">Model quality</div><h2 className="font-display text-2xl text-[#133458]">Performance by depth</h2></div><div className="flex gap-1 bg-[#f1edc9] p-1">{['By depth', 'By region'].map((x) => <button onClick={() => setTab(x)} key={x} className={`px-3 py-2 text-[11px] ${tab === x ? 'bg-[#133458] text-[#FAF7BB]' : 'text-[#536675]'}`} data-testid={`button-performance-tab-${x.toLowerCase().replace(' ', '-')}`}>{x}</button>)}</div></div><div className="mt-6 overflow-x-auto"><table className="w-full min-w-[590px] border-collapse text-left"><thead><tr className="border-b border-[#D8D0B3] font-data text-[9px] uppercase tracking-wider text-[#8a927f]"><th className="pb-3">Depth band</th><th className="pb-3">Profiles</th><th className="pb-3">RMSE</th><th className="pb-3">Accuracy</th><th className="pb-3">Confidence</th></tr></thead><tbody>{depthValues.map((row, i) => <tr key={row.depth} className="border-b border-[#D8D0B3]/60 text-xs"><td className="py-4 font-semibold text-[#133458]">{tab === 'By depth' ? row.depth : ['Arabian Sea', 'North Atlantic', 'South Pacific', 'Kuroshio', 'Southern Ocean'][i]}</td><td className="py-4 font-data text-[#536675]">{[4821, 3924, 5580, 3288, 1771][i].toLocaleString()}</td><td className="py-4 font-data text-[#133458]">{row.rmse}°C</td><td className="py-4 font-data text-[#133458]">{row.score}%</td><td className="py-4"><span className="inline-flex items-center gap-1.5 text-[#536b35]"><span className="h-1.5 w-1.5 rounded-full bg-[#838921]" /> reliable</span></td></tr>)}</tbody></table></div></section><section className="oe-card mt-5 p-5"><div className="oe-kicker mb-1">Benchmark</div><div className="flex flex-wrap items-end justify-between gap-3"><h2 className="font-display text-2xl text-[#133458]">Model comparison</h2><span className="font-data text-[9px] text-[#8a927f]">HELD-OUT ARGO VALIDATION SET · N=4,920</span></div><div className="mt-5 overflow-x-auto"><table className="w-full min-w-[530px] text-left"><thead><tr className="border-b border-[#D8D0B3] font-data text-[9px] uppercase tracking-wider text-[#8a927f]"><th className="pb-3">Model</th><th className="pb-3">RMSE</th><th className="pb-3">R²</th><th className="pb-3">Inference</th><th className="pb-3">Status</th></tr></thead><tbody>{[['OceanEmbed v2.4', '0.82°C', '0.947', '1.2 s', 'Selected'], ['OceanEmbed v2.1', '0.97°C', '0.921', '0.8 s', 'Archived'], ['Climatology baseline', '1.44°C', '0.806', '0.1 s', 'Baseline']].map(([a,b,c,d,e], i) => <tr key={a} className="border-b border-[#D8D0B3]/60 text-xs"><td className="py-4 font-semibold text-[#133458]">{a}</td><td className="py-4 font-data">{b}</td><td className="py-4 font-data">{c}</td><td className="py-4 font-data">{d}</td><td className={`py-4 ${i === 0 ? 'text-[#838921]' : 'text-[#8a927f]'}`}>{e}</td></tr>)}</tbody></table></div></section></div></AppShell>;
}

function Dataset() {
  const [query, setQuery] = useState('');
  const sources = useMemo(() => [['Argo float profiles', 'In situ temperature and salinity', '2000–2024', '24,892,104', 'Global'], ['Sentinel-3 SLSTR', 'Sea surface temperature', '2016–2024', '2.1 PB', 'Global'], ['MODIS Aqua', 'Ocean color and chlorophyll-a', '2002–2024', '684 TB', 'Global'], ['ECMWF ORAS5', 'Ocean reanalysis prior', '1958–2023', '341 GB', 'Global'], ['HYCOM GLBy0.08', 'Physical ocean state', '1994–2024', '1.8 TB', 'Atlantic / Pacific']].filter((s) => s.join(' ').toLowerCase().includes(query.toLowerCase())), [query]);
  return <AppShell><div className="oe-page"><SectionHeading eyebrow="Source registry" title="Dataset" description="A traceable inventory of the satellite, in situ, and reanalysis sources behind every OceanEmbed inference." action={<button className="oe-control oe-primary flex items-center gap-2" data-testid="button-add-source"><Plus size={15} /> Add source</button>} /><div className="grid gap-5 lg:grid-cols-[1fr_285px]"><section className="oe-card p-5"><div className="flex flex-wrap items-center justify-between gap-3"><div className="relative"><Search size={15} className="absolute left-3 top-2.5 text-[#8a927f]" /><input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search sources" className="h-9 w-64 border border-[#D8D0B3] bg-[#FAF7BB] pl-9 pr-3 text-xs outline-none focus:border-[#838921]" data-testid="input-search-dataset" /></div><button className="oe-control flex items-center gap-2" data-testid="button-filter-dataset"><SlidersHorizontal size={14} /> Filters</button></div><div className="mt-5 overflow-x-auto"><table className="w-full min-w-[650px] text-left"><thead><tr className="border-b border-[#D8D0B3] font-data text-[9px] uppercase tracking-wider text-[#8a927f]"><th className="pb-3">Source</th><th className="pb-3">Coverage</th><th className="pb-3">Period</th><th className="pb-3">Volume</th><th className="pb-3">Status</th></tr></thead><tbody>{sources.map((s, i) => <tr key={s[0]} className="border-b border-[#D8D0B3]/60 text-xs"><td className="py-4"><div className="font-semibold text-[#133458]">{s[0]}</div><div className="mt-1 text-[10px] text-[#536675]">{s[1]}</div></td><td className="py-4 text-[#536675]">{s[4]}</td><td className="py-4 font-data text-[10px]">{s[2]}</td><td className="py-4 font-data text-[10px]">{s[3]}</td><td className="py-4"><span className="flex items-center gap-1.5 text-[#536b35]"><span className="h-1.5 w-1.5 rounded-full bg-[#838921]" /> Synced</span></td></tr>)}</tbody></table>{sources.length === 0 && <div className="py-14 text-center text-sm text-[#536675]">No sources match “{query}”.</div>}</div></section><aside className="space-y-5"><div className="oe-card bg-[#133458] p-5 text-[#FAF7BB]"><div className="oe-kicker !text-[#D99B21]">Coverage index</div><div className="mt-4 font-data text-4xl">98.6%</div><p className="mt-2 text-xs leading-5 text-[#FAF7BB]/60">Of the global ocean has at least one valid observation in the current window.</p><div className="mt-5 h-1 bg-[#FAF7BB]/15"><div className="h-full w-[98.6%] bg-[#D99B21]" /></div></div><div className="oe-card p-5"><div className="oe-kicker mb-1">Data freshness</div><h2 className="font-display text-2xl text-[#133458]">Last sync</h2><div className="mt-5 flex items-center gap-3"><Cloud size={20} className="text-[#838921]" /><div><div className="font-data text-sm text-[#133458]">03 JUN · 09:42 UTC</div><div className="mt-1 text-[11px] text-[#536675]">All source pipelines nominal</div></div></div><button className="mt-5 flex items-center gap-2 text-xs font-semibold text-[#838921]" data-testid="button-refresh-dataset"><RotateCcw size={13} /> Check for updates</button></div></aside></div></div></AppShell>;
}

function Architecture() {
  const nodes: Array<{ n: string; title: string; sub: string; icon: Icon }> = [
    { n: '01', title: 'Satellite field', sub: '14 channels', icon: Globe2 },
    { n: '02', title: 'Patch encoder', sub: 'ViT · 12 layers', icon: Network },
    { n: '03', title: 'Ocean context', sub: 'Cross-attention', icon: Waves },
    { n: '04', title: 'Depth decoder', sub: '2,000 m output', icon: Mountain },
  ];
  return <AppShell><div className="oe-page oe-grid"><SectionHeading eyebrow="Model card · OceanEmbed v2.4" title="Architecture" description="A visual record of the representation learning stack used to resolve subsurface thermal structure." action={<button className="oe-control flex items-center gap-2" data-testid="button-download-model-card"><Download size={15} /> Download model card</button>} /><div className="oe-card overflow-hidden"><div className="border-b border-[#D8D0B3] bg-[#133458] px-5 py-4 text-[#FAF7BB]"><div className="flex flex-wrap items-center justify-between gap-3"><div className="font-data text-[10px] tracking-[.1em]">OCEANEMBED / ENCODER–DECODER / V2.4.1</div><span className="flex items-center gap-2 text-[10px] text-[#FAF7BB]/60"><span className="h-1.5 w-1.5 rounded-full bg-[#D99B21]" /> production</span></div></div><div className="overflow-x-auto p-8"><div className="flex min-w-[850px] items-center justify-center gap-3">{nodes.map(({ n, title, sub, icon: ArchitectureIcon }, i) => <div className="flex items-center gap-3" key={title}><div className="w-[172px] border border-[#D8D0B3] bg-[#f5f1d6] p-4"><div className="flex items-center justify-between"><span className="font-data text-[10px] text-[#D99B21]">{n}</span><ArchitectureIcon size={17} className="text-[#838921]" /></div><div className="mt-8 text-xs font-semibold text-[#133458]">{title}</div><div className="mt-1 font-data text-[9px] text-[#536675]">{sub}</div><div className="mt-4 flex gap-1">{[1,2,3,4,5].map((x) => <span key={x} className={`h-1.5 flex-1 ${x <= i + 2 ? 'bg-[#838921]' : 'bg-[#D8D0B3]'}`} />)}</div></div>{i < 3 && <ArrowRight className="shrink-0 text-[#D99B21]" size={18} />}</div>)}</div></div><div className="grid gap-0 border-t border-[#D8D0B3] sm:grid-cols-3"><div className="border-b border-[#D8D0B3] p-5 sm:border-b-0 sm:border-r"><div className="oe-label">Parameters</div><div className="mt-2 font-data text-xl text-[#133458]">86.4M</div></div><div className="border-b border-[#D8D0B3] p-5 sm:border-b-0 sm:border-r"><div className="oe-label">Embedding dimension</div><div className="mt-2 font-data text-xl text-[#133458]">768</div></div><div className="p-5"><div className="oe-label">Training window</div><div className="mt-2 font-data text-xl text-[#133458]">1993–2024</div></div></div></div></div></AppShell>;
}
